...
布隆过滤器在判断一个元素不存在时，是始终准确的；判断存在时，存在一定的误判率。
*/
...
int main()
{
    ...
    std::string url = "http://www.alibaba.com";
    std::string url1 = "http://www.cencent.com";
    std::cout << std::boolalpha << list.query(url) << std::endl;
    std::cout << std::boolalpha << list.query(url1) << std::endl;
    return 0;
}
